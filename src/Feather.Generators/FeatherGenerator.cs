...
public sealed class FeatherGenerator : IIncrementalGenerator
{
    ...
    private static string GenerateGraphicsPipeline(GraphicsPipelineModel pipeline)
    {
        ...
        var vertexIr = Convert.ToBase64String(CompressIr(FeatherIrWriter.WriteModule(pipeline.VertexShader)));
        var fragmentIr = Convert.ToBase64String(CompressIr(FeatherIrWriter.WriteModule(pipeline.FragmentShader)));

...
public sealed class FeatherGenerator : IIncrementalGenerator
{
    ...
    private static string GenerateKernel(ShaderModel model)
    {
        ...
        var emission = FeatherIrWriter.EmitModule(model);
        var ir = FeatherIrWriter.ToCSharpByteArray(emission.Module);
        var sourceMap = FeatherIrSourceMapWriter.ToBase64(model, emission);
        ...
        return $$"""
            ...
            {{accessors}}
                ...
                private const string __feather_source_map = "{{sourceMap}}";
    ...
    private static byte[] CompressIr(byte[] ir)
    {
    ...
    private static string GenerateGraphicsPipeline(GraphicsPipelineModel pipeline)
    {
        ...
        var vertexEmission = FeatherIrWriter.EmitModule(pipeline.VertexShader);
        var fragmentEmission = FeatherIrWriter.EmitModule(pipeline.FragmentShader);
        var vertexIr = Convert.ToBase64String(CompressIr(vertexEmission.Module));
        var fragmentIr = Convert.ToBase64String(CompressIr(fragmentEmission.Module));
        var vertexSourceMap = FeatherIrSourceMapWriter.ToBase64(pipeline.VertexShader, vertexEmission);
        var fragmentSourceMap = FeatherIrSourceMapWriter.ToBase64(pipeline.FragmentShader, fragmentEmission);
        var vertexIrField = "__feather_vertex_ir" + accessorSuffix;
        var fragmentIrField = "__feather_fragment_ir" + accessorSuffix;
        var vertexSourceMapField = "__feather_vertex_source_map" + accessorSuffix;
        var fragmentSourceMapField = "__feather_fragment_source_map" + accessorSuffix;
        var decodeIrMethod = "DecodeIr" + accessorSuffix;
        ...
        return $$"""
            ...
            {
            {{vertexAccessors}}
                private static readonly byte[] {{vertexIrField}} = {{decodeIrMethod}}("{{vertexIr}}");
                private static readonly byte[] {{fragmentIrField}} = {{decodeIrMethod}}("{{fragmentIr}}");
                private const string {{vertexSourceMapField}} = "{{vertexSourceMap}}";
                private const string {{fragmentSourceMapField}} = "{{fragmentSourceMap}}";
            ...
            {{resources}}
                ...
                {
                    using var compressed = new global::System.IO.MemoryStream(global::System.Convert.FromBase64String(encoded));
                    using var stream = new global::System.IO.Compression.GZipStream(
                        compressed, global::System.IO.Compression.CompressionMode.Decompress);
                    using var expanded = new global::System.IO.MemoryStream();
    ...
    private static string GenerateGraphicsPushConstantBinding(IReadOnlyList<ResourceModel> stageResources, IReadOnlyList<ResourceModel> pipelineResources, string shaderParameterName, string accessorSuffix)
    {
        ...
        var builder = new StringBuilder();
        builder.Append("        var __feather_push_constant_total_size = ")
            .Append(totalSize)
            .AppendLine(";");
        var offsets = new Dictionary<string, string>(StringComparer.Ordinal);
        var offset = "0";
        for (var i = 0; i < pipelinePushConstants.Length; i++)
        {
            var resource = pipelinePushConstants[i];
            offset = AlignOffsetExpression(offset, PushConstantAlignmentExpression(resource));
            var local = "__feather_push_constant_offset_" + i;
            builder.Append("        var ").Append(local).Append(" = ").Append(offset).AppendLine(";");
            offsets[resource.Binding + ":" + resource.Name] = local;
        ...
        for (var i = 0; i < pushConstants.Length; i++)
        {
            var resource = pushConstants[i];
            var size = PushConstantSizeExpression(resource);
            if (!offsets.TryGetValue(resource.Binding + ":" + resource.Name, out var offsetLocal))
            {
                throw new InvalidOperationException("Graphics push constant was not found in the pipeline resource list.");
            }
            builder.Append("        var __feather_push_constant_value_").Append(i).Append(" = ")
            ...
            builder.Append("        command.SetPushConstantRange((uint)(")
                ...
                .Append(i)
                .Append(", (uint)(")
                .Append("__feather_push_constant_total_size")
                .AppendLine("));");
        }

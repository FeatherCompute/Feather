...
public sealed class FeContextHandle : FeSafeHandle
{
    protected override bool ReleaseHandle()
    {
        ...
        return IsInvalid || NativeMethods.IsProcessExiting || NativeMethods.fe_context_shutdown_raw(handle).Succeeded();
    }
}
...
public sealed class FeTextureHandle : FeSafeHandle
{
    ...
    protected override bool ReleaseHandle() => IsInvalid || NativeMethods.IsProcessExiting || NativeMethods.fe_texture_destroy(handle).Succeeded();
}

public sealed class FeSamplerHandle : FeSafeHandle
{
    protected override bool ReleaseHandle() => IsInvalid || NativeMethods.IsProcessExiting || NativeMethods.fe_sampler_destroy(handle).Succeeded();
}

public sealed class FeKernelHandle : FeSafeHandle
{
    protected override bool ReleaseHandle() => IsInvalid || NativeMethods.IsProcessExiting || NativeMethods.fe_kernel_destroy(handle).Succeeded();
}

public sealed class FeGraphicsPipelineHandle : FeSafeHandle
{
    protected override bool ReleaseHandle() => IsInvalid || NativeMethods.IsProcessExiting || NativeMethods.fe_graphics_pipeline_destroy(handle).Succeeded();
}
...
{
    protected override bool ReleaseHandle() => IsInvalid || NativeMethods.IsProcessExiting || NativeMethods.fe_window_destroy(handle).Succeeded();
}
...
{
    internal FeResult ReleaseSubmission()
    {
        ...
        var result = NativeMethods.fe_fence_destroy_raw(DangerousGetHandle());
        if (result.Succeeded())
        {
            SetHandleAsInvalid();
        }

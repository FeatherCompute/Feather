...
/// through Luisa's XIR automatic-differentiation pipeline.
/// </summary>
    ...
    where TKernel : struct, IKernel1D, IGeneratedKernel<TKernel>
{
    ...
    public GpuADKernel(TKernel kernel, GpuExecutionBackend backend)
        : this(kernel, ValidateBackend(backend))
    {
    }

    private GpuADKernel(TKernel kernel, bool validatedBackend)
    {
        _ = validatedBackend;
        Kernel = kernel;
        gpuKernel = GpuKernel.Create<TKernel>(GPU.Context, autoDiff: true);
    ...
    /// <summary>
    /// Runs the forward pass, generates the XIR adjoint kernel, dispatches the backward pass,
    ...
    private static bool ValidateBackend(GpuExecutionBackend backend)
        => backend == GpuExecutionBackend.Luisa
            ? true
